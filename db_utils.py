...
def init_db():
    ...
    try:
        ...
        try:
            ...
            cursor.execute(
                "ALTER TABLE sessions ADD COLUMN user_identifier TEXT NOT NULL DEFAULT 'default_user'"
            )
...
def get_sessions(user_identifier: str):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT session_id, created_at, last_updated FROM sessions WHERE user_identifier = ? ORDER BY last_updated DESC",
            (user_identifier,),
        )
        sessions = [dict(row) for row in cursor.fetchall()]
        logger.info(
            f"DEBUG: Found {len(sessions)} sessions for user {user_identifier}."
        )
    except sqlite3.Error as e:
...
def load_session(session_id: str, user_identifier: str):
    ...
    try:
        ...
        cursor.execute(
            "SELECT history FROM sessions WHERE session_id = ? AND user_identifier = ?",
        ...
        if result and result[0]:
            ...
            logger.info(
                f"DEBUG: Session {session_id} loaded successfully for user {user_identifier}, history length: {len(history)}"
            )
        elif result is None:
            logger.warning(
                f"DEBUG: Session {session_id} not found or does not belong to user {user_identifier}."
            )
        else:  # result[0] is None or empty
            logger.warning(
                f"DEBUG: No history found or empty for session: {session_id}"
            )
...
def save_session(session_id: str, user_identifier: str, history: list):
    ...
    logger.info(
        f"DEBUG: Saving session: {session_id} for user: {user_identifier}, history length: {len(history)}"
    )
    now = datetime.now()
    ...
    try:
        ...
        conn.commit()
        logger.info(
            f"DEBUG: Session {session_id} saved successfully for user {user_identifier}."
        )
    except sqlite3.Error as e:
        logger.error(
            f"Error saving session {session_id} for user {user_identifier}: {e}"
        )
    finally:
        if conn:
            conn.close()
